use nix::libc;
use std::{ fs::File, os::fd::AsRawFd, mem };
...
impl Config {
    ...
    pub fn configure(&mut self) {
        ...
        unsafe {
            ...
            oss_set_format(fd, &mut self.format).expect("Failed to set format");
            oss_set_speed(fd, &mut self.rate).expect("Failed to set sample rate");
        ...
        self.bytes = buffer_info.bytes;
        self.channels = audio_info.max_channels;
        self.samples = self.bytes / mem::size_of::<i32>() as i32;
        self.chsamples = self.samples / self.channels;
    }
}
